...
public function main() returns error? {
    ...
    sb:Message|sb:Error? messageReceived = subscriptionReceiver->receive(serverWaitTime);

    if (messageReceived is sb:Message) {
        check subscriptionReceiver->renewLock(messageReceived);
        sb:Message|sb:Error? messageReceivedAgain = subscriptionReceiver->receive(serverWaitTime);
