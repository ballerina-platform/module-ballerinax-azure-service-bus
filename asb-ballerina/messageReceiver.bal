...
public isolated client class MessageReceiver {
    ...
    public isolated function init(string connectionString, string entityPath, string receiveMode = PEEKLOCK, int poolSize = DEFAULT_POOL_SIZE) {
        self.connectionString = connectionString;
        self.entityPath = entityPath;
        self.receiveMode = receiveMode;
        handle receiverHandle = initMessageReceiver(java:fromString(self.connectionString), 
        java:fromString(self.entityPath), java:fromString(self.receiveMode), poolSize);
    }
    ...
    isolated remote function receive(@display {label: "Server Wait Time"} int? serverWaitTime = 60) 
        ...
        Message|() message = check receive(serverWaitTime);
    ...
    isolated remote function receiveBatch(@display {label: "Maximum Message Count"} int maxMessageCount, 
        ...
        MessageBatch|() receivedMessages = check receiveBatch(maxMessageCount, serverWaitTime);
    ...
    isolated remote function complete(@display {label: "Message"} Message message) 
        ...
        if (message?.lockToken.toString() != DEFAULT_MESSAGE_LOCK_TOKEN) {
            return complete(message?.lockToken.toString());
        }
    ...
    isolated remote function abandon(@display {label: "Message"} Message message) returns Error? {
        if (message?.lockToken.toString() != DEFAULT_MESSAGE_LOCK_TOKEN) {
            return abandon(message?.lockToken.toString());
        }
    ...
    isolated remote function deadLetter(@display {label: "Message"} Message message, 
        ...
        if (message?.lockToken.toString() != DEFAULT_MESSAGE_LOCK_TOKEN) {
            return deadLetter(message?.lockToken.toString(), deadLetterReason, 
    ...
    isolated remote function defer(@display {label: "Message"} Message message) 
        ...
        _ = check defer(message?.lockToken.toString());
    ...
    isolated remote function receiveDeferred(@display {label: "Deferred Msg Seq Num"} 
        ...
        Message|() message = check receiveDeferred(sequenceNumber);
        if (message is Message) {
            _ = check self.mapContentType(message);
            return message;
        }
    ...
    isolated remote function renewLock(@display {label: "Message"} Message message) returns Error? {
        if (message?.lockToken.toString() != DEFAULT_MESSAGE_LOCK_TOKEN) {
            return renewLock(message?.lockToken.toString());
        }
    ...
    isolated remote function setPrefetchCount(@display {label: "Prefetch Count"} int prefetchCount) returns Error? {
        return setPrefetchCount(prefetchCount);
    }
    ...
    isolated remote function close() returns Error? {
        var connectionResult = closeReceiver();
...
isolated function initMessageReceiver(handle connectionString, handle entityPath, handle receiveMode, int poolSize) returns handle = @java:Constructor {
    'class: "org.ballerinax.asb.receiver.MessageReceiver",
    paramTypes: ["java.lang.String", "java.lang.String", "java.lang.String", "int"]
} external;

isolated function receive(int? serverWaitTime) returns Message|Error? = @java:Method {
    name: "receive",
    'class: "org.ballerinax.asb.receiver.MessageReceiver"
} external;

isolated function receiveBatch(int? maxMessageCount, int? serverWaitTime) 
                               returns MessageBatch|Error? = @java:Method {
    name: "receiveBatch",
    'class: "org.ballerinax.asb.receiver.MessageReceiver"
} external;

isolated function complete(string? lockToken) returns Error? = @java:Method {
    name: "complete",
    'class: "org.ballerinax.asb.receiver.MessageReceiver"
} external;

isolated function abandon(string? lockToken) returns Error? = @java:Method {
    name: "abandon",
    'class: "org.ballerinax.asb.receiver.MessageReceiver"
} external;

isolated function deadLetter(string? lockToken, string? deadLetterReason, 
                             string? deadLetterErrorDescription) returns Error? = @java:Method {
    name: "deadLetter",
    'class: "org.ballerinax.asb.receiver.MessageReceiver"
} external;

isolated function defer(string? lockToken) returns Error? = @java:Method {
    name: "defer",
    'class: "org.ballerinax.asb.receiver.MessageReceiver"
} external;

isolated function receiveDeferred(int sequenceNumber) returns Message|Error? = @java:Method {
    name: "receiveDeferred",
    'class: "org.ballerinax.asb.receiver.MessageReceiver"
} external;

isolated function renewLock(string? lockToken) returns Error? = @java:Method {
    name: "renewLock",
    'class: "org.ballerinax.asb.receiver.MessageReceiver"
} external;

isolated function setPrefetchCount(int prefetchCount) returns Error? = @java:Method {
    name: "setPrefetchCount",
    'class: "org.ballerinax.asb.receiver.MessageReceiver"
} external;

isolated function closeReceiver() returns Error? = @java:Method {
    name: "closeReceiver",
    'class: "org.ballerinax.asb.receiver.MessageReceiver"
} external;
