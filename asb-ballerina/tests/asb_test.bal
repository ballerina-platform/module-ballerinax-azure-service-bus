...
function testSendAndReceiveMessageFromQueueOperation() {
    ...
    MessageSender messageSender = new (connectionString, "myqueue");
    checkpanic messageSender->send(message1);
    MessageSender messageSender2 = new (connectionString, "mytest");
    checkpanic messageSender2->send(message1);
    checkpanic messageSender->send(message1);


    log:printInfo("Creating Asb message receiver.");
    MessageReceiver messageReceiver = new (connectionString, queueName, RECEIVEANDDELETE);
