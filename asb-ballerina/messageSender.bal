...
public isolated client class MessageSender {
    ...
    public isolated function init(string connectionString, string entityPath, int poolSize = DEFAULT_POOL_SIZE) {
        self.connectionString = connectionString;
        self.entityPath = entityPath;
        handle senderHandle = newMessageSender(java:fromString(self.connectionString), java:fromString(self.entityPath), poolSize);
    }
    ...
    isolated remote function send(@display {label: "Message"} Message message) returns error? {
        ...
        if (message.body is byte[]) {
            response = check send(message.body, message?.contentType, 
            ...
            response = check send(messageBodyAsByteArray, message?.contentType, 
                message?.messageId, message?.to, message?.replyTo, message?.replyToSessionId, message?.label, 
                message?.sessionId, message?.correlationId, message?.partitionKey, message?.timeToLive, 
                message?.applicationProperties?.properties);
        }
    ...
    isolated remote function close() returns error? {
        error? connectionResult = closeSender();
...
isolated function newMessageSender(handle connectionString, handle entityPath, int poolSize) returns handle = @java:Constructor {
    'class: "org.ballerinax.asb.sender.MessageSender",
    paramTypes: ["java.lang.String", "java.lang.String", "int"]
} external;

isolated function send(string|xml|json|byte[] body, string? contentType, 
...
isolated function sendBatch(MessageBatch messages) returns error? = @java:Method {
    name: "sendBatch",
    'class: "org.ballerinax.asb.sender.MessageSender"
} external;

isolated function closeSender() returns error? = @java:Method {
    name: "closeSender",
    'class: "org.ballerinax.asb.sender.MessageSender"
} external;
