...
import org.ballerinax.asb.IMessageSenderFactory;
...
public class MessageSender {
    ...
    private IMessageSenderPool pool;
    private String entityPath;
    private IMessageSenderFactory factory = new IMessageSenderFactory();

    public MessageSender(String connectionString, String entityPath, int poolSize) {
        this.entityPath = entityPath;
        factory.addMessageSenderConnectionString(entityPath, connectionString);
        // TODO: Assign poolSize to a variable.
    }
    ...
    public Object send(Object body, Object contentType, Object messageId,
        ...
        try {
            sender = IMessageSenderPool.getInstance(factory).borrowObject(entityPath);
            ...
            IMessageSenderPool.getInstance(factory).returnObject(entityPath, sender);
        }
    }
    ...
    public Object closeSender() {
        try {
            IMessageSenderPool.getInstance(factory).invalidateObject(entityPath, sender);
            return null;
        } catch (Exception e) {
            return ASBUtils.returnErrorValue("Exception occurred while invalidating the sender object" + e.getMessage());
        }
    }
}
